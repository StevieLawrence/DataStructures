...
Date: 6/24/17
Purpose: Create a maxHeap class
"""
...
class maxHeap(object):
    ...
    def __init__(self):
        """
        Create a max heap which is a binary tree with the root values being greater
        than the children.  Heaps increase left to right across a level before 
        a new level is filled.
        """
        self.__theHeap = [None]
        self.__size = 0
    ...
    def getParent(self, x):
        """
        Gets the parental node of the node at position x if it is a valid position. 
        if the position is not valie then return None.
    ...
    def percolateUp(self, x):
        """
        Continously swap position x with its parent and its parent's parent... until the payload is larger than the children.
           x - position within the heap
        """
        # initialize parent
    ...
    def percolateDn(self, x):
        """
            Continously swap position x with its parent and its parents parent... until the payload is larger than the children.
               x - position within the heap
            """
        ...
        while (lChild is not None):
            ...
            # Go left if there is no right child or if the left child is larger than the right child
    ...
    def deleteMin(self):
        """
        return the value of the root or maximum value.  The root is the replaces with the last leaf of the heap and is
    ...
    def heapify(self, aList):
        ...
        down from there.  It continues this all the way up until it hits the root.
